...
TC4: Verify seach - no input 
    [Documentation]    Verify search - no input
    Enter page shop directly
    Seach product no keyword

TC5: Verify search absolute - with input (product name)
...
TC6: Verify search absolute - with input (brand)
...
TC7: Verify search absolute - with input (category)
...
TC14: Verify filter category - laptops
...
TC15: Verify filter category - fragrances
...
TC16: Verify filter category - skincare
...
TC17: Verify filter category - groceries
...
TC18: Verify filter category - home decoration
...
TC19: Verify filter category - furniture
...
TC29: Verify filter price range - 200-500
...
TC30: Verify filter price range - 500-1000
...
TC31: Verify filter price range > 1000
...
TC32: Verify filter order by - price - desc
...
TC33: Verify filter order by - discount rate - desc
...
TC34: Verify filter order by - item title - desc
...
TC35: Verify filter order by - rating - desc
...
TC36: Verify filter order by - price - asc
    [Documentation]    Verify filter order by - price
    Enter page shop directly
    Filter order by    price
...
TC37: Verify filter order by - discount rate - asc
    [Documentation]    Verify filter order by - discount rate
    Enter page shop directly
    Filter order by    discount rate
    Sort product list    asc
...
TC38: Verify filter order by - item title - asc
    [Documentation]    Verify filter order by - item title
    Enter page shop directly
    Filter order by    item title
    Sort product list    asc
...
TC39: Verify filter order by - rating - asc
    [Documentation]    Verify filter order by - rating
    Enter page shop directly
    Filter order by    rating
    Sort product list    asc
...
TC40: Verify multiple filter (category, price range, order by)
...
TC41: Verify multiple filter (search, price range, order by)
...
### Verify reset filter ###
TC42: Verify reset filter (category, price range, order by)
...
TC43: Verify reset filter (search, price range, order by)
    [Documentation]    Verify reset filter
    Enter page shop directly
    Search product    wrist
    Filter by price range    0
...
TC44: Verify reset filter (search, category, order by)
    [Documentation]    Verify reset filter
    Enter page shop directly
    Search product    book
    Filter by category    laptops    1
...
TC45: Verify reset filter (search, category, price range, order by, sort)
    [Documentation]    Verify reset filter
    Enter page shop directly
